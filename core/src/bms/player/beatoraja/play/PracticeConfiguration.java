...
public final class PracticeConfiguration {
	...
	private final PracticeModeControls controls = new PracticeModeControls();
	...
	public void draw(Rectangle r, SkinObjectRenderer sprite, long time, MainState state) {
		final Color unfocusedColor = controls.isHorizontalMode() ? Color.GRAY : Color.CYAN;
		final Color focusedColor = controls.getInputTurbo() ? Color.ORANGE : Color.YELLOW;
	...
	enum PracticeElement {
		...
		STARTTIME((practice, inc, isAnalog, turboSpeed) -> {
			...
			final int maxStartTime = Math.max(0, roundDownTo(tl[tl.length - 1].getTime() - 2000, 100));
