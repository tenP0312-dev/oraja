		...
		try (Archive rar = open(archive)) {
			...
			for (FileHeader header : headers) {
				...
				if (size < 0 || totalSize + size < totalSize || totalSize + size > MAX_EXTRACTED_SIZE) {
					throw new IOException("RAR is too large after decompression: " + archive);
				}
				totalSize += size;
		...
		try {
			...
			try {
				for (FileHeader header : rar.getFileHeaders()) {
					validate(header, archive);
					if (!header.isDirectory() && entryName.equals(normalizeEntryNameOrNull(header.getFileName()))) {
						InputStream stream = new FilterInputStream(rar.getInputStream(header)) {
							@Override
						...
						return limitStream(stream, header.getFullUnpackSize(),
								"RAR resource is too large: " + entryName);
					}
				}
				rar.close();
				throw new IOException("RAR entry does not exist: " + entryName);
