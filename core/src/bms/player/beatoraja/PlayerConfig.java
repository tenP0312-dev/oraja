...
public final class PlayerConfig {
	...
	/** Use difficulty-table entry levels for Music Select display and sorting. */
	...
	private boolean bmsirJudgeRankSortSkinNoticeEnabled = true;
	...
	public boolean isBmsirTableLevelDisplayEnabled() {
		return bmsirTableLevelDisplayEnabled;
	}

	public void setBmsirTableLevelDisplayEnabled(boolean enabled) {
		bmsirTableLevelDisplayEnabled = enabled;
	}
	...
	public int[] getBmsirHispeedReferenceBpms() {
		Mode[] modes = bmsirSpecificPlayModes();
		int[] values = new int[modes.length];
		for (int index = 0; index < modes.length; index++) {
			values[index] = getPlayConfig(modes[index])
					.getPlayconfig()
					.getBmsirHispeedReferenceBpm();
		}
		return values;
	}

	public void setBmsirHispeedReferenceBpms(int[] values) {
		if (values == null) {
			return;
		}
		Mode[] modes = bmsirSpecificPlayModes();
	...
	public void setBmsirJudgeRankSortEnabled(boolean enabled) {
		bmsirJudgeRankSortEnabled = enabled;
		int judgeSort = Arrays.asList(BarSorter.defaultSorter).indexOf(
				BarSorter.JUDGE
		);
		if (!enabled && (sort == judgeSort
				|| BarSorter.JUDGE.name().equals(sortid))) {
			sort = Arrays.asList(BarSorter.defaultSorter).indexOf(
					BarSorter.TITLE
			);
			sortid = BarSorter.TITLE.name();
		}
	}

	public boolean isBmsirJudgeRankSortSkinNoticeEnabled() {
		return bmsirJudgeRankSortSkinNoticeEnabled;
	}
