...
class MusicSelectConstantTest {
    @Test
    void assistKeyTogglesBpmRemovalWithoutChangingOrajaConstantAndPersists() {
        for (int scroll = 0; scroll <= 2; scroll++) {
            for (boolean constant : new boolean[]{false, true}) {
                ...
                MusicSelectInputProcessor.toggleRegulSpeed(player);
                PlayerConfig restored = new Json().fromJson(PlayerConfig.class, PlayerConfig.getConfigJson(player));
                assertEquals(scroll == 1 ? 0 : 1, restored.getScrollMode());
                ...
                MusicSelectInputProcessor.toggleRegulSpeed(restored);
                assertEquals(scroll == 1 ? 1 : 0, restored.getScrollMode());
            }
        }
    }
