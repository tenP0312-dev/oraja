...
`Arena oraja 0.4.14.77`. It replaces the separate Endless Dream and
...
perspective-correct textured meshes, projective nested masks, color
transforms, and named blend modes. Invalid input fails safely, retained
resources are bounded and disposed, and skins without a scene keep the
existing SpriteBatch path. Launcher `0.2.29`, plugin `0.0.72`, protocol v8,
and all `0.4.14.76` behavior remain unchanged.
...
```text
BMS-IR-Arena-oraja-0.4.14.77-macos-aarch64.jar
```
...
python tools/package_arena_release.py \
  ...
  --body-jar dist/BMS-IR-Arena-oraja-0.4.14.77-macos-aarch64.jar \
...
python tools/package_arena_release.py \
  ...
  --body-jar dist/BMS-IR-Arena-oraja-0.4.14.77-windows-x86-64.jar \
  --plugin-jar /reviewed/bms_ir_arena_oraja_0.0.72.jar \
  --base-assets /reviewed/clean-beatoraja-assets \
